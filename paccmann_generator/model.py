...
class Model:
    ...
    def create_combined_model(self, params, params_o, params_p, omics_df, protein_df, logger, model_folder_name):
        ...
        generator_smiles_language = SMILESLanguage.load(
            os.path.join(params['mol_model_path'], 'selfies_language.pkl')
        )

        #load predictors
        with open(os.path.join(params['ic50_model_path'], 'model_params.json')) as f:
            paccmann_params = json.load(f)

        paccmann_predictor = MODEL_FACTORY_OMICS['mca'](paccmann_params)
        paccmann_predictor.load(
            os.path.join(
                params['ic50_model_path'],
        ...
        paccmann_smiles_language = SMILESLanguage.load(
            os.path.join(params['ic50_model_path'], 'smiles_language.pkl')
        )
        paccmann_predictor._associate_language(paccmann_smiles_language)

        with open(os.path.join(params['affinity_model_path'], 'model_params.json')) as f:
        ...
        affinity_smiles_language = SMILESLanguage.load(
            os.path.join(params['affinity_model_path'], 'smiles_language.pkl')
        )
        affinity_protein_language = ProteinLanguage.load(
            os.path.join(params['affinity_model_path'], 'protein_language.pkl')
        )
        ...
        with open(os.path.join(params['mol_model_path'], 'model_params.json')) as f:
        ...
        generator_rl.load(
            os.path.join(
                params['mol_model_path'], f"weights/best_{params.get('metric', 'rec')}.pt"
            ),
            map_location=get_device()
        )
        generator_rl.eval()
        ...
        # Restore omics model
        with open(os.path.join(params['omics_model_path'], 'model_params.json')) as f:
            cell_params = json.load(f)
        cell_encoder_rl = ENCODER_FACTORY['dense'](cell_params)
        cell_encoder_rl.load(
            os.path.join(
                params['omics_model_path'],
        ...
        if (model_folder_name is None):
            model_folder_name = site + '_' + self.type + '_' + params['model_name'] + '_combined'
        if self.type =='set':
            ...
            with open(os.path.join(params['set_encoder_path'], 'train_params.json')) as f:
                encoder_params = json.load(f)
            #rename model keys
            new_state_dict = {}
            checkpoint = torch.load(params['set_encoder_path']+'/setsae', map_location=device)
            ...
            combined = ReinforceProteinOmics( 
                ...
                model_folder_name, logger, params['remove_invalid'], ensemble_type=self.type, set_encoder=encoder_rl)
        else:
            combined = ReinforceProteinOmics(
                ...
                generator_smiles_language, model_folder_name, logger, params['remove_invalid'], ensemble_type=self.type
            )
        return combined
